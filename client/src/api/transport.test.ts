...
import {
  ...
  RETRYABLE_WITH_OWN_BACKOFF,
  RETRYABLE_WITH_SERVER_INTERVAL,
  type FetchLike,
} from './transport.ts';
...
describe('retries — idempotent verbs only, capped, jittered', () => {
  ...
  // ── THE STATUS POLICY, DERIVED FROM THE SETS THAT IMPLEMENT IT ─────────────────────────────
  //
  // The cases below are generated by ITERATING `RETRYABLE_WITH_OWN_BACKOFF` and
  // `RETRYABLE_WITH_SERVER_INTERVAL`, not by restating their members. A restated list agrees
  // with the code today and constrains nothing tomorrow: adding a status to a set would leave
  // the list passing, untouched and silent. Derived cases mean a new member is exercised
  // immediately and must behave as its set says.
  //
  // `EXPECTED_CLASSIFICATION` then closes the loop in BOTH directions, so the sets cannot drift
  // from the documented policy either:
  //   * every member of either set must appear here with the matching classification — a status
  //     added to a set without a decision recorded here fails;
  //   * every status named here must actually be in the set its classification says — a status
  //     REMOVED from a set, or moved between them, fails.
  const EXPECTED_CLASSIFICATION: ReadonlyMap<number, 'own-backoff' | 'server-interval' | 'never'> =
    new Map([
      [408, 'own-backoff'],
      [502, 'own-backoff'],
      [504, 'own-backoff'],
      // Retried only on a server-supplied interval — guessing a backoff against a server that
      // declined to name one is how a thundering herd starts.
      [429, 'server-interval'],
      [503, 'server-interval'],
      // Never retried, with or without a Retry-After, and not rescued by an explicit opt-in.
      [400, 'never'],
      [401, 'never'],
      [409, 'never'],
      [500, 'never'],
  ...
  const NEVER_RETRIED = [...EXPECTED_CLASSIFICATION]
    .filter(([, classification]) => classification === 'never')
    .map(([status]) => status);

  /** Attempt the same status twice: once with no `Retry-After`, once with one. */
  async function attemptCounts(status: number): Promise<{ bare: number; withHeader: number }> {
    ...
    return { bare: bare.calls.length, withHeader: withHeader.calls.length };
  }

  it('classifies every status in either retryable set, and only those it says it does', () => {
    for (const status of RETRYABLE_WITH_OWN_BACKOFF) {
      expect(EXPECTED_CLASSIFICATION.get(status), `status ${status} is retried on the transport's `
        + 'own backoff but has no recorded classification').toBe('own-backoff');
    }
    for (const status of RETRYABLE_WITH_SERVER_INTERVAL) {
      expect(EXPECTED_CLASSIFICATION.get(status), `status ${status} is retried on a server `
        + 'interval but has no recorded classification').toBe('server-interval');
    }
    for (const [status, classification] of EXPECTED_CLASSIFICATION) {
      expect(RETRYABLE_WITH_OWN_BACKOFF.has(status), `status ${status}`)
        .toBe(classification === 'own-backoff');
      expect(RETRYABLE_WITH_SERVER_INTERVAL.has(status), `status ${status}`)
        .toBe(classification === 'server-interval');
    }
    // The two sets are disjoint: a status cannot be governed by both rules at once.
    for (const status of RETRYABLE_WITH_OWN_BACKOFF) {
      expect(RETRYABLE_WITH_SERVER_INTERVAL.has(status), `status ${status}`).toBe(false);
    }
  });

  it.each([...RETRYABLE_WITH_OWN_BACKOFF])(
    'retries %i on the transport\'s own backoff, with or without Retry-After',
    async (status) => {
      expect(await attemptCounts(status)).toEqual({ bare: 2, withHeader: 2 });
    },
  );

  it.each([...RETRYABLE_WITH_SERVER_INTERVAL])(
    'retries %i ONLY when the server supplies a usable Retry-After',
    async (status) => {
      expect(await attemptCounts(status)).toEqual({ bare: 1, withHeader: 2 });
    },
  );

  it.each(NEVER_RETRIED)('never retries %i, with or without Retry-After', async (status) => {
    expect(await attemptCounts(status)).toEqual({ bare: 1, withHeader: 1 });
  });

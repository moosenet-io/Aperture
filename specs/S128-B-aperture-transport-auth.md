...
- **Context:** Sprint A produced a repo, a design system, a BFF skeleton, and a versioned
  ...
  1. **Streaming is greenfield in the emitting direction only — the consuming direction already
     ...
     **Consequence for every item below:** do not write a second SSE frame parser, do not add a
     new incremental-output hook to the agent loop, and do not reimplement the idle-timeout or
     buffered-fallback semantics. Reuse them, and map their existing outcomes onto the Aperture
     event taxonomy.
...
- **Description:** Implement `GET /v1/aperture/stream` in the BFF as a Server-Sent Events
  ...
  **Scope boundary — read this before writing a line.** The core already *consumes* SSE from
  ...
  - `contracts/aperture-api-v1.yaml` — the stream endpoint's media type, query parameters, and
  ...
  1. Model the event set as a **Rust enum with one variant per event type**, serialized to the
     ...
     truth, not the code.
  2. Every event carries: `seq` (monotonic, APTR-16), `stream_id`, `thread_id`, `ts`, and a
     type-specific payload. `token` carries the delta text only — never the accumulated
     buffer — so a long response does not degrade to O(n²) bytes on the wire.
  3. `message.start` opens a logical assistant turn and carries the message id the client will
     ...
     shutdown path must emit `message.end` or `error` first.
  ...
  5. `thinking` carries reasoning-visible content where the named proxy supplies it. It is
     **opt-in per thread** and defaults off — never surfaced unless the client asked for it.
  ...
  - Integration: `thinking` events are absent unless explicitly requested for the thread
...
- **Acceptance criteria:**
  ...
  - [ ] Every stream terminates with `message.end` or `error` — no silent truncation on any path
  - [ ] Tool call/result payloads sanitized (keys redacted, >1KB truncated) before framing
  ...
  - [ ] All anti-buffering headers present; `thinking` is opt-in and off by default
...
- **Description:** A mobile client on a flaky connection will drop a stream mid-generation.
  ...
  4. If the requested `seq` has aged out of the buffer, do **not** silently skip. Emit a
     `context` event marking a replay gap with the range lost, then resume live — and let the
     client decide whether to refetch the turn via REST. Silent gaps are worse than declared ones.
  5. If the `stream_id` is entirely unknown (server restarted, stream reaped), respond with a
     fresh stream and an explicit gap marker rather than a 4xx — the client should recover, not
     error out.
  6. Resume must be **authorization-checked on every reconnect**, not trusted from the id. A
  ...
  7. Retire buffers on `message.end` plus a short grace window, and on stream reap — an
     unbounded map of dead streams is a memory leak with a long fuse.
  ...
  - Integration: reconnect with an aged-out id yields an explicit gap marker, then live events
  - Integration: reconnect with an unknown `stream_id` yields a fresh stream plus a gap marker,
    not a 4xx
  - Verify no hardcoded IPs, hostnames, ports, or org names in new/modified files
...
- **Acceptance criteria:**
  - [ ] `seq` is strictly monotonic per stream and emitted as the SSE `id:` field
  - [ ] Replay buffer is bounded by both event count and bytes, configured by name not literal
  - [ ] Resume with a valid `Last-Event-ID` yields no gap and no duplicate
  - [ ] Aged-out and unknown-stream resumes emit an explicit gap marker, never a silent skip
  - [ ] Resume is authorization-checked per reconnect; cross-user resume is refused
  - [ ] Dead-stream buffers are reaped; no unbounded growth
  - [ ] No hardcoded infrastructure values in new/modified code
  - [ ] All existing tests still pass
...
- **Description:** When a user closes a tab, navigates away, or presses stop mid-generation, the
  ...
  ## APPROACH
  1. Every turn executes under a cancellation token owned by the stream. Client disconnect,
     explicit stop, device revocation (APTR-23), and stream reap (APTR-17) all trip the **same**
     token — one cancellation mechanism, not four.
  ...
  ## TEST PLAN
  - Integration: disconnect mid-generation; assert the upstream request is cancelled within the
    bounded detection window
  - Integration: explicit stop mid-generation; assert `message.end` with reason `cancelled`
  ...
  - Verify no hardcoded IPs, hostnames, ports, or org names in new/modified files
  - **Negative:** assert that after disconnect, **zero** further upstream inference tokens are
    requested — the orphaned-generation regression test, and the reason this item exists
  ...
  - A client that reconnects with `Last-Event-ID` **after** cancelling — resume must observe the
    cancelled terminal, not restart generation
...
- **Acceptance criteria:**
  - [ ] Disconnect, explicit stop, revocation, and reap all trip one shared cancellation token
  - [ ] Cancellation propagates to the upstream inference request; zero orphaned generation
  - [ ] Disconnect is detected proactively, not only on the next failed write
  - [ ] Partial turns persist with a `cancelled` marker and still reach the memory flush path
  - [ ] Exactly one terminal event per turn; post-cancellation tokens are discarded
  - [ ] Server-side teardown completes even when the client is already gone
  - [ ] No hardcoded infrastructure values in new/modified code
  - [ ] All existing tests still pass
...
- **Description:** The client half of the transport. A single, well-tested stream consumer that
  ...
  1. Build on the platform `EventSource` where its semantics suffice, and fall back to a
     ...
     APTR-07 rule that `fetch` lives in one place. No absolute URLs — same-origin relative only.
  ...
  4. Honour the server's explicit gap marker (APTR-16) by exposing a `gap` flag in the store so
     the UI can refetch the turn over REST. **A gap is surfaced, never smoothed over.**
  ...
  - Integration: simulated disconnect mid-turn resumes and yields the same final state as an
    uninterrupted run — the core correctness property of the whole transport
  - Verify no hardcoded IPs, hostnames, ports, or org names in new/modified files
  ...
  - Reconnect storm from many tabs after a backend restart — jitter plus a per-origin connection
    guard; assert reconnects are spread, not simultaneous
  - Browser `EventSource` auto-reconnecting on its own schedule and fighting the custom backoff —
    own the reconnect explicitly, or disable the built-in and document which
...
- **Acceptance criteria:**
  ...
  - [ ] Resume via `Last-Event-ID` reproduces the identical final state as an uninterrupted stream
  - [ ] Duplicates dropped, in-window reorders corrected, unrecoverable gaps surfaced explicitly
  - [ ] Reducer is pure and exhaustive; a new contract event type fails typechecking until handled
  - [ ] Transport constructed in exactly one file; same-origin relative URLs only
...
- **Description:** Aperture today has no way to create its first account. Implement the first-run
  ...
  6. On success, issue a session (APTR-21) and mark bootstrap `complete` durably. Re-invoking
     bootstrap after completion returns a forbidden problem-details response and **is rate
     limited and audit-logged** (APTR-24).
...
- **Acceptance criteria:**
  - [ ] Bootstrap runs only when zero admins exist, guarded transactionally against races
  - [ ] Bootstrap closes permanently after completion; re-attempts are refused and audit-logged
...
- **Description:** Aperture is the first surface where anyone other than the operator can
  ...
  7. Every role change, invite creation, invite acceptance, and revocation is audit-logged with
     actor, target, and timestamp — with arguments sanitized per APTR-10.
...
- **Description:** With web, desktop, and mobile targets, one user will routinely hold four or
  ...
  6. A user may revoke only their own devices. An admin may revoke another user's devices, and
     every such cross-user action is audit-logged with actor and target.
...
- **Description:** The auth endpoints are the instance's only unauthenticated attack surface.
  ...
  4. Cover login, refresh, invite acceptance, bootstrap, and password change. Deliberately do
  ...
  8. Sustained abuse is audit-logged once per window with a counter, not once per attempt —
     per-attempt logging is itself an amplification vector against the log.
...
- **Acceptance criteria:**
  - [ ] Per-source and per-account limiting both enforced, tripping independently
...
- **Description:** Matrix gives read receipts and per-device read markers for free, and the moment
  ...
  4. Client writes are **debounced and coalesced** — scrolling emits one write per settle window,
     not per frame. Optimistic local update, reconciled from the server's echo.
...
- **Acceptance criteria:**
  - [ ] Read position stored per user-per-thread; unread counts derived, never stored separately
...
- **Description:** Implement the epic's channel policy table exactly, no more and no less.
  ...
  *names*, and a test asserting it stays inert.
  ...
  3. **Telegram documentation** is the real deliverable here: what it is for, its privacy posture
  ...
  4. **Signal — inert skeleton.** Implement `Channel` with `name()` returning `"signal"`, a `start()`
  ...
  5. Signal's module descriptor reports `unavailable` with the reason "deferred by operator
     decision", so the shell renders an inert explained tile per APTR-08 rather than a broken screen.
  6. `ChannelType` gains a `Signal` variant only if it can be added without implying support; if in
     doubt, leave Signal out of the identity enum entirely until it is real. Record the decision in
     `docs/CHANNELS.md`.
  7. Every channel appears in the module descriptor list with an accurate capability state, so the
...
- **Acceptance criteria:**
  ...
  - [ ] Telegram is runtime-selectable and **off by default**; off means not started, not connected
